...
use super::addrinfo::{getaddrinfo, AddrInfoHints};
use super::nameinfo::getnameinfo;

/// Lookup the address for a given hostname via DNS.
///
/// Returns an iterator of IP Addresses, or an `io::Error` on failure.
pub fn lookup_host(host: &str) -> Result<Vec<IpAddr>, String> {
...
fn test_hostname() {
  ...
  let hostname = super::hostname::get_hostname().unwrap();
